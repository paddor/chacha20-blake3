...
// # Safety
//
// Caller must ensure that AVX-512F is available (e.g. via `is_x86_feature_detected!("avx512f")`).
// The annotation enables the compiler to auto-vectorize the XOR and keystream-extraction
// loops with AVX-512 instructions, not just the explicit intrinsic calls.
#[target_feature(enable = "avx512f")]
pub unsafe fn chacha_avx512<const ROUNDS: usize>(
    state: &mut [u32; STATE_WORDS],
    input: &mut [u8],
    last_keystream_block: &mut [u8; BLOCK_SIZE],
) {
    // SAFETY: AVX-512F availability is guaranteed by #[target_feature] + caller contract.
    unsafe { chacha_avx512_inner::<ROUNDS>(state, input, last_keystream_block) }
}

/// Inner function keeps the original code structure with a single `unsafe` scope.
#[target_feature(enable = "avx512f")]
unsafe fn chacha_avx512_inner<const ROUNDS: usize>(
    state: &mut [u32; STATE_WORDS],
    input: &mut [u8],
    last_keystream_block: &mut [u8; BLOCK_SIZE],
) {
...
#[target_feature(enable = "avx512f")]
unsafe fn chacha20_avx512_16blocks<const ROUNDS: usize>(
